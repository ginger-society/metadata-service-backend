...
use NotificationService::apis::configuration::{ApiKey, Configuration}; // Adjust based on your crate structure
use NotificationService::get_configuration; // Assuming get_configuration exists and returns Configuration

#[derive(Debug)]
pub struct NotificationService_api_config(pub Configuration); // Wrapper struct for Configuration

#[rocket::async_trait]
impl<'r> FromRequest<'r> for NotificationService_api_config {
    type Error = ();
    ...
    async fn from_request(request: &'r Request<'_>) -> Outcome<Self, Self::Error> {
        ...
        Outcome::Success(NotificationService_api_config(configuration))
    }
}

impl<'a> OpenApiFromRequest<'a> for NotificationService_api_config {
